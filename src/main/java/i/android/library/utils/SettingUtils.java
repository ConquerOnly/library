...
public final class SettingUtils {
    ...
    private static Method findApplyMethod() {
        try {
            ...
            if (DEBUG) {
                LogUtil.d(TAG, "Failed to retrieve Editor.apply(); probably doesn't exist on this phone's OS.  Using Editor.commit() instead.");
            }
            return null;
        }
    }
    ...
    public static void commitOrApply(Editor editor) {
        if (APPLY_METHOD != null) {
            try {
                ...
                if (DEBUG) {
                    LogUtil.d(TAG, "Failed while using Editor.apply().  Using Editor.commit() instead.");
                }
